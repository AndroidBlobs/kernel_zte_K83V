				     ...
				     size_t count, loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		error = tfaContWriteFile(tfa98xx->handle, msg_file, 0, 0); /* int vstep_idx, int vstep_msg_idx both 0 */
		if (error)
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->value.integer.max = max(0, tfacont_get_max_vstep(tfa98xx->handle, profile) - 1);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx->handle == 0) {
		...
		if (ret == Tfa98xx_Error_Ok){
			...
			pr_err("dll+++++ret == ret == Tfa98xx_Error_Not_Supported\n");
			tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL;
			}
		mutex_unlock(&tfa98xx->dsp_lock);
	}
	...
	if (out1) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTNOCLK, out1) > 0) {
			...
			if ((tfa98xx->handle == 0) && (no_clk == 1)) {
				...
				err = tfa98xx_tfa_start(tfa98xx, sw_profil, tfa98xx_vsteps);
				if (err != Tfa98xx_Error_Ok) {
                 ...
                 */
				if (strstr(tfaContProfileName(tfa98xx->handle, sw_profil), ".saam")) {
