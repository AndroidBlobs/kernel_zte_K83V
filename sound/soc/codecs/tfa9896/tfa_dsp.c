...
static void tfa_set_query_info (int dev_idx)
{
	...
	case 0x97:
		...
		pr_err("dll+++test tfa_set_query_info  id is 96\n");
		break;
	case 0x96:
		/* tfa9896 */
		handles_local[dev_idx].supportDrc = supportNo;
	...
	case 0x12:
		...
		handles_local[dev_idx].spkr_count = 1;
		handles_local[dev_idx].daimap = Tfa98xx_DAI_I2S;
		if (tfa9887_is87(dev_idx) == 1) /***MCH_TO_TEST***/
		{
...
enum Tfa98xx_Error tfa98xx_dsp_system_stable(Tfa98xx_handle_t dev_idx, int *ready)
{
	...
	if (handles_local[dev_idx].dev_ops.tfa_dsp_system_stable){
		...
		/* generic function */
		pr_err("dll+++++test+_tfa98xx_dsp_system_stable, dev_idx : %d!!!\n", dev_idx);
	...
	if(handles_local[handle].dev_ops.dsp_msg) {
		...
		if(handles_local[handle].ext_dsp == 1) {
				...
				if(lastmessage == 1) {
					...
					length = tfa_tib_dsp_msgblob(-1, length, (const char*)blob);

					if (tfa98xx_runtime_verbose)
						pr_err("Last message for the multi-message received. Multi-message length=%d \n", length);

					/* Send tot the target selected */
					error = (*handles_local[handle].dev_ops.dsp_msg)(handle, length, (const char*)blob);
					if(error != Tfa98xx_Error_Ok)
			...
			error = (*handles_local[handle].dev_ops.dsp_msg)(handle, length, buf);
			if(error != Tfa98xx_Error_Ok)
				error = (enum Tfa98xx_Error) (error + Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */
		}
	} else {
...
enum Tfa98xx_Error dsp_msg_read(Tfa98xx_handle_t handle, int length, unsigned char *bytes)
{
	...
	if(handles_local[handle].dev_ops.dsp_msg_read) {
		error = (*handles_local[handle].dev_ops.dsp_msg_read)(handle, length, bytes);
		if(error != Tfa98xx_Error_Ok)
			error = (enum Tfa98xx_Error) (error + Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */
	} else {
...
enum Tfa98xx_Error mem_read(Tfa98xx_handle_t handle, unsigned int start_offset, int num_words, int *pValues)
{
	enum Tfa98xx_Error error;

	if(handles_local[handle].dev_ops.mem_read) {
		error = (*handles_local[handle].dev_ops.mem_read)(handle, start_offset, num_words, pValues);
		if(error != Tfa98xx_Error_Ok)
			error = (enum Tfa98xx_Error) (error + Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */
	} else {
...
/*Nxp patch for no sound whe boost begin*/
/*
	...
	*/
/*Nxp patch for no sound whe boost end*/
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	if ( !force ) { // in case of force CF already runnning
		...
		if (tfa_cf_enabled(handle) == 0)
			return err;
		pr_err("dll++++tfa+11111++tfaRunSpeakerStartup\n");
		if(!handles_local[handle].ext_dsp) {
			pr_err("dll++++tfa+222222++tfaRunSpeakerStartup\n");
...
enum Tfa98xx_Error tfaRunSpeakerCalibration(Tfa98xx_handle_t handle, int profile)
{
	...
	if ((strstr(tfaContProfileName(handle, profile), ".cal") == NULL)
...
enum Tfa98xx_Error tfaRunStartup(Tfa98xx_handle_t handle, int profile)
{
    ...
    switch (TFA_GET_BF(handle, REV) & 0xff)
    {
        case 0x80:
            err = tfa98xx_dsp_reset(handle, 1);
            pr_err("tfaRunStartup Reset DSP after power on\n");
            break;
        default:
            break;
    }
	...
	if (!handles_local[handle].ext_dsp) {
		...
		if (tfa_cf_enabled(handle)) {
			...
			if (tries == CFSTABLE_TRIES) {
				if (tfa98xx_runtime_verbose) pr_err("Timed out\n");
				return Tfa98xx_Error_StateTimedOut;
			}  else
				if (tfa98xx_runtime_verbose) pr_err(" OK (tries=%d)\n", tries);
		}
	}
	err = tfa98xx_read_register16(handle, 0x00, &value);
	pr_err("dll+++SYS_STATUS0: 0x%04x\n", value);
...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	pr_err("dll++++tfa+++tfaRunColdStartup\n");
	err = tfaRunStartup(handle, profile);
	PRINT_ASSERT(err);
	if (err)
		return err;
	pr_err("dll++++tfa++2+tfaRunColdStartup\n");
	if((handles_local[handle].rev & 0xFF) != 0x72) {
		pr_err("dll++tfa  RunColdStartup++33+handles_local[handle].rev & 0xFF) != 0x72");
		/* force cold boot */
	...
	pr_err("dll++++tfa+444++tfaRunColdStartup\n");
	/* start */
	err = tfaRunStartDSP(handle);
	PRINT_ASSERT(err);
...
enum Tfa98xx_Error tfaRunWaitCalibration(Tfa98xx_handle_t handle, int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	while ((*calibrateDone != 1) && (tries<TFA98XX_API_WAITRESULT_NTRIES)) {
		err = mem_read(handle, TFA_FW_XMEM_CALIBRATION_DONE, 1, calibrateDone);
		tries++;
	}
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if(((handles_local[dev].rev & 0xff) == 0x72) && (!handles_local[dev].ext_dsp)) {
...
enum Tfa98xx_Error tfa_mtp_get_calibration(Tfa98xx_handle_t handle, uint16_t cal_data[1])
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;

	cal_data[0] = (uint16_t)tfa_read_reg(handle, (uint16_t)TFA_MK_BF(0xF5,0,16));
