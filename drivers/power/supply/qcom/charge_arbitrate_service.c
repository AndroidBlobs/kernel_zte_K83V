...
#define FULL_RAW_SOC							100
#define ICL_TARGET_CURRENT					100000
...
static int charge_arbitrate_debug_battery_info(struct charge_arbitrate_info *cas_info)
{
	...
	rc = power_supply_get_property(cas_info->subsidiary_psy,
...
{
	union power_supply_propval input_limited = {0, };
	int rc = 0, target_ua = ICL_TARGET_CURRENT;

	target_ua = flag ? ICL_TARGET_CURRENT : 0;

	rc = power_supply_get_property(cas_info->battery_psy,
					POWER_SUPPLY_PROP_INPUT_CURRENT_LIMITED, &input_limited);
	if (rc < 0) {
		pr_alarm("Failed to get input limited property rc=%d\n", rc);
		goto failed_loop;
	}

	if (input_limited.intval != target_ua) {
		input_limited.intval = target_ua;
		pr_alarm("set input limited to %d\n", flag);
		rc = power_supply_set_property(cas_info->battery_psy,
					POWER_SUPPLY_PROP_INPUT_CURRENT_LIMITED, &input_limited);
		if (rc < 0) {
			pr_alarm("Failed to set input limited property rc=%d\n", rc);
			goto failed_loop;
		}
	} else {
		pr_alarm("input limited status already is %d\n", target_ua);
	}
