...
#define CHARGING_EXPIRATION_TIME_MS_24 86400 /*24 hours*/
#endif
...
#ifdef ZTE_SPECIAL_CHARGER
	policy_info->config_prop.timeout_seconds = CHARGING_EXPIRATION_TIME_MS_24;
#endif
...
static int charger_policy_notifier_handler(struct charger_policy_info *policy_info)
{
	...
	pr_policy("soc=%d, temp=%d, policy_lowtemp_en=%d, charging_policy_status=0x%x,"
		"expired[%d,%d] demo[%d,%d], old status: %s charging\n",
...
#if defined(CONFIG_ZTE_CHARGER_TYPE_OEM)
	if ((policy_info->battery_charging_policy_ops.charging_policy_status & DEMO_CHARGING_POLICY)) {
	...
	} else if ((policy_info->battery_charging_policy_ops.charging_policy_status & EXPIRED_CHARGING_POLICY)) {
		charger_policy_status = true;
		charger_policy_ctrl_cas(true);
		...
		unsigned long event, void *data)
{
	...
	if ((strcmp(name, "usb") == 0)) {
		...
		if (policy_info->usb_present ^ usb_pressent_val.intval) {
			queue_delayed_work(policy_info->timeout_workqueue, &policy_info->timeout_work,
				msecs_to_jiffies(0));
		}
	}
...
static int charger_policy_probe(struct platform_device *pdev)
{
	...
	goto parse_dt_failed;
#else
	if (!policy_info->config_prop.turn_on || socinfo_get_charger_flag()) {
