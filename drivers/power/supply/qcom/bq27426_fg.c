...
struct bq_fg_chip {
	...
	struct wakeup_source *fg_poll_wake_lock;
...
static int fg_set_prop_enable_charging(struct bq_fg_chip *bq, bool enable)
{
	...
	ret = batt_psy->desc->set_property(batt_psy,
...
static int fg_psy_register(struct bq_fg_chip *bq)
{
	...
	bq->fg_psy_d.external_power_changed = fg_external_power_changed;
...
static void fg_charge_enable_workfunc(struct work_struct *work)
{
	...
	if (ret) {
		alarm_start_relative(&bq->charge_enable_alarm,
...
static irqreturn_t fg_irq_thread(int irq, void *dev_id)
{
	...
	if (!bq->resume_completed) {
		...
		__pm_wakeup_event(bq->fg_poll_wake_lock, 1 * HZ);
		if (!bq->irq_disabled) {
...
/*	fg_change_chem_id(bq, 3142); */
	...
	bq->fg_poll_wake_lock = wakeup_source_register("fg_poll_wake");
	if (!bq->fg_poll_wake_lock)
		return -EINVAL;
