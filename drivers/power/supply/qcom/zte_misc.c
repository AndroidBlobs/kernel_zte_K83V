...
static char *hw_id_text[] = {
	"undefined", "u8xA", "u8xB", "u8xC", "u8xD", "u8xE", "u8xF"
};
...
static int zte_misc_screen_on_get(char *val, const struct kernel_param *kp)
{
	...
	return snprintf(val, sizeof(int), "%d",  screen_on);
}

static struct kernel_param_ops zte_misc_screen_on_ops = {
	.set = zte_misc_screen_on_set,
	.get = zte_misc_screen_on_get,
};
module_param_cb(screen_on, &zte_misc_screen_on_ops, &screen_on, 0644);
