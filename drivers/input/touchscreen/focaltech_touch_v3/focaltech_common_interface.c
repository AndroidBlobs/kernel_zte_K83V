...
static int tpd_get_glove_mode(struct tpd_classdev_t *cdev)
{
	cdev->b_glove_enable = fts_data->glove_mode;
	return 0;
}

static int tpd_enable_glove_mode(struct tpd_classdev_t *cdev, int enable)
{
	int ret = 0;
...
int tpd_register_fw_class(struct fts_ts_data *data)
{
	...
	#endif
	tpd_fw_cdev.get_glove_mode = tpd_get_glove_mode;
	tpd_fw_cdev.set_glove_mode = tpd_enable_glove_mode;
