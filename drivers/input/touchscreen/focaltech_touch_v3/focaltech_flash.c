...
*****************************************************************************/
extern int get_fts_module_info_from_lcd(void);
...
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg)
{
	...
	if ((ret < 0) || (!upg->module_info)) {
		upg->module_id = get_fts_module_info_from_lcd();
		if (upg->module_id > 0) {
			for (i = 0; i < FTS_GET_MODULE_NUM; i++) {
				upg->module_info = &module_list[i];
				if (upg->module_id == upg->module_info->id) {
					FTS_INFO("module id 0x%x get module info pass from lcd", upg->module_id);
					break;
				}
			}
			if (i >= FTS_GET_MODULE_NUM) {
