...
static ssize_t fts_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t count);
static ssize_t fts_gesture_buf_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t fts_gesture_buf_store(struct device *dev, struct device_attribute *attr, const char *buf,
				     size_t count);

int tpd_get_gesture_mode(void)
{
	FTS_INFO("%s wakeup_gesture_enable val is:%d.\n", __func__, fts_gesture_data.mode);
	return fts_gesture_data.mode;
}

int tpd_set_gesture_mode(int gesture_mode)
{

	FTS_INFO("%s previous val is:%d, current val is:%d.\n", __func__, fts_gesture_data.mode,
		 gesture_mode);

	fts_gesture_data.mode = gesture_mode;
	return 0;
}
 ...
 */
static DEVICE_ATTR(fts_gesture_buf, 0644, fts_gesture_buf_show, fts_gesture_buf_store);
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
	...
	case GESTURE_LEFT:
		gesture = KEY_GESTURE_LEFT;
		FTS_DEBUG("fts Light up screen by GESTURE_LEFT gesture wakeup.\n");
		break;
	case GESTURE_RIGHT:
		gesture = KEY_GESTURE_RIGHT;
		FTS_DEBUG("fts Light up screen by GESTURE_RIGHT gesture wakeup.\n");
		break;
	case GESTURE_DOUBLECLICK:
		gesture = KEY_GESTURE_DOUBLEC;
		FTS_DEBUG("fts Light up screen by DOUBLE CLICK gesture wakeup.\n");
		break;
	case GESTURE_C:
		gesture = KEY_GESTURE_C;
		FTS_DEBUG("fts Light up screen by GESTURE C gesture wakeup.\n");
		break;
	case GESTURE_O:
		gesture = KEY_GESTURE_O;
		FTS_DEBUG("fts Light up screen by GESTURE O gesture wakeup.\n");
		break;
	case GESTURE_W:
		gesture = KEY_GESTURE_W;
		FTS_DEBUG("fts Light up screen by GESTURE W gesture wakeup.\n");
		break;
	case GESTURE_M:
		gesture = KEY_GESTURE_M;
		FTS_DEBUG("fts Light up screen by GESTURE M gesture wakeup.\n");
		break;
	default:
		gesture = -1;
		break;
...
int fts_gesture_readdata(struct fts_ts_data *ts_data)
{
	...
	buf = kmalloc((FTS_GESTRUE_POINTS * sizeof(u16)), GFP_KERNEL);
	if (buf == NULL) {
	...
	memset(buf, 0, FTS_GESTRUE_POINTS * sizeof(u16));
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
	...
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_LEFT);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_RIGHT);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_DOUBLEC);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_C);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_M);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_O);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_W);
