...
int ini_get_key(char *filedata, char *section, char *key, char *value)
{
	...
	for (i = 0; i < test_data.ini_keyword_num; i++) {
		...
		if (strlen(key) == test_data.ini_data[i].key_name_len) {
			if (fts_strncmp
			    (key, test_data.ini_data[i].key_name, test_data.ini_data[i].key_name_len) == 0)
				/* test_data.ini_data[i].key_name_len) == 0) */
			{
				memcpy(value, test_data.ini_data[i].key_value,
				       test_data.ini_data[i].key_value_len);
				ret = 0;
				break;
			}
		}
...
char *ini_str_trim_r(char *buf)
{
	...
	len = strlen(buf) + 1; /* strlcpy will add '/0' at end of buf */
/*tmp = (char *)malloc(len);*/
...
static int ini_file_get_line(char *filedata, char *buffer, int maxlen)
{
	...
	for (i = 0, j = 0; i < maxlen; j++) {
		...
		if (ch1 == '\n' || ch1 == '\r') {
			...
			goto READ_END;	/*line breaks*/
		} else if (ch1 == 0x00) {
			iRetNum = -1;
			goto READ_END;	/*file end*/
		} else {
...
int print_key_data(void)
{
	...
	for (i = 0; i < MAX_KEYWORD_NUM; i++) {
		...
		FTS_TEST_DBG("section_name_%d:%s, key_name_%d:%s\n,key_value_%d:%s",
			     ...
			     i, test_data.ini_data[i].key_name, i, test_data.ini_data[i].key_value);
	}
...
int ini_get_key_data(char *filedata)
{
	...
	while (1) {		/*find section */
		...
		if (buf1[0] == CFG_SSL) {
			...
			if (MAX_KEYWORD_NAME_LEN <
			    test_data.ini_data[test_data.ini_keyword_num].section_name_len) {
				ret = CFG_ERR_OUT_OF_LEN;
				FTS_TEST_ERROR("MAX_KEYWORD_NAME_LEN: CFG_ERR_OUT_OF_LEN\n");
				goto cfg_scts_end;
			}
			buf1[n - 1] = 0x00;
			strlcpy((char *)tmsection_name, buf1 + 1, n);

			continue;
		}
		/* get section name end */
		strlcpy(test_data.ini_data[test_data.ini_keyword_num].section_name,
			tmsection_name, strlen(tmsection_name) + 1);
		...
		if (test_data.ini_data[test_data.ini_keyword_num].key_name_len > MAX_KEYWORD_NAME_LEN) {
			ret = CFG_ERR_OUT_OF_LEN;
			FTS_TEST_ERROR("MAX_KEYWORD_NAME_LEN: CFG_ERR_OUT_OF_LEN\n");
			goto cfg_scts_end;
		}
		...
		if (test_data.ini_data[test_data.ini_keyword_num].key_value_len > MAX_KEYWORD_VALUE_LEN) {
			ret = CFG_ERR_OUT_OF_LEN;
		...
		if (test_data.ini_keyword_num > MAX_KEYWORD_NUM) {
...
int GetPrivateProfileString(char *section, char *ItemName, char *defaultvalue, char *returnValue,
			    char *IniFile)
{
	char value[512] = { 0 };
	int len = 0;
	...
	if (ini_get_key(IniFile, section, ItemName, value) < 0) {
		...
		snprintf(returnValue, MAX_PATH, "%s", value);
	} else {
		len = snprintf(returnValue, MAX_PATH, "%s", value);
		return len;
	}
	return 0;
...
int fts_test_get_ini_size(char *config_name)
{
	...
	off_t fsize = 0;
	char filepath[128];
	char ini_file_path[INI_FILE_PATH_MAX_SZIE];

	FTS_TEST_FUNC_ENTER();

	get_ini_file_path(ini_file_path);
	memset(filepath, 0, sizeof(filepath));
	snprintf(filepath, sizeof(filepath), "%s%s", ini_file_path, config_name);
...
int init_interface(char *ini)
{
	...
	/*Get IC Name */
	snprintf(test_data.ini_ic_name, 20, "%s", str);
	FTS_TEST_INFO("IC Name:%s IC Code:0x%02x. ", test_data.ini_ic_name,
		      test_data.screen_param.selected_ic);
...
/************************************************************************
* Name: set_param_data
* Brief:  load Config. Set IC series, init test items, init basic threshold, int detailThreshold,
	and set order of test items
