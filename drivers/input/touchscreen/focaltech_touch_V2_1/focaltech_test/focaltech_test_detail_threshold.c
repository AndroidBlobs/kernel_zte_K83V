...
int malloc_struct_DetailThreshold(void)
{
	...
	if (test_data.incell_detail_thr.cb_test_min == NULL)
		goto ERR;
	test_data.incell_detail_thr.cb_test_max = fts_malloc(NUM_MAX * sizeof(int));
	if (test_data.incell_detail_thr.cb_test_max == NULL)
		goto ERR;
...
void free_struct_DetailThreshold(void)
{
	...
	if (test_data.incell_detail_thr.rawdata_test_b_frame_min != NULL) {
		fts_free(test_data.incell_detail_thr.rawdata_test_b_frame_min);
		test_data.incell_detail_thr.rawdata_test_b_frame_min = NULL;
	}
	if (test_data.incell_detail_thr.rawdata_test_b_frame_max != NULL) {
...
void OnInit_DThreshold_RawDataTest(char *strIniFile)
{
	...
	if ((TEST_ICSERIES(test_data.screen_param.selected_ic) == TEST_ICSERIES(IC_FT8716))
	    || (TEST_ICSERIES(test_data.screen_param.selected_ic) == TEST_ICSERIES(IC_FT8736))
	    || (TEST_ICSERIES(test_data.screen_param.selected_ic) == TEST_ICSERIES(IC_FTE716))) {
		return;
	}
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			...
			test_data.incell_detail_thr.rawdata_test_b_frame_max[i * test_data.screen_param.rx_num + j] =
				B_MaxValue;
		}
	}
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				...
				test_data.incell_detail_thr.rawdata_test_max[i *
									     test_data.screen_param.rx_num + k]
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			...
			test_data.incell_detail_thr.rawdata_test_min[i * test_data.screen_param.rx_num + j] =
				MinValue;
			test_data.incell_detail_thr.rawdata_test_b_frame_min[i * test_data.screen_param.rx_num + j] =
				B_MinValue;
		}
	}
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				...
				test_data.incell_detail_thr.rawdata_test_min[i * test_data.screen_param.rx_num + k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.rawdata_test_low_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.rawdata_test_low_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.rawdata_test_high_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.rawdata_test_high_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_SCapRawDataTest(char *strIniFile)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_rawdata_off_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_rawdata_off_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	 ...
	 /*ON*/ GetPrivateProfileString("Basic_Threshold", "SCapRawDataTest_ON_Min", "150", str, strIniFile);
	MinValue = fts_atoi(str);
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_rawdata_on_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_rawdata_on_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_SCapCbTest(char *strIniFile)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_cb_test_on_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_cb_test_on_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_cb_test_off_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.scap_cb_test_off_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_PanelDifferTest(char *strIniFile)
{
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.panel_differ_test_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.panel_differ_test_min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_RxLinearityTest(char *strIniFile)
{
	...
	for (i = 0; i < test_data.screen_param.used_max_tx_num; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.rx_linearity_test_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_TxLinearityTest(char *strIniFile)
{
	...
	for (i = 0; i < test_data.screen_param.used_max_tx_num; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.tx_linearity_test_max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_ForceTouch_SCapRawDataTest(char *strIniFile)
{
	...
	/* OFF */
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_OFF_Min", "150", str,
				strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_OFF_Max", "1000", str,
				strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapRawDataTest_OFF_Max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapRawDataTest_OFF_Min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	/* ON */
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_ON_Min", "150", str,
				strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_ON_Max", "1000", str,
				strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapRawDataTest_ON_Max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapRawDataTest_ON_Min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_ForceTouch_SCapCbTest(char *strIniFile)
{
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapCbTest_ON_Max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapCbTest_ON_Min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapCbTest_OFF_Max[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.ForceTouch_SCapCbTest_OFF_Min[i][k] =
				    (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_CBTest(char *strIniFile)
{
	...
	FTS_TEST_FUNC_ENTER();
	if (TEST_ICSERIES(test_data.screen_param.selected_ic) == TEST_ICSERIES(IC_FT8606)) {
		return;
	}
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			test_data.incell_detail_thr.cb_test_max[i * test_data.screen_param.rx_num + j] =
			    MaxValue;
		}

		if (i == ChannelNumTest_ChannelXNum) {
			for (j = 0; j < test_data.screen_param.rx_num; j++) {
				test_data.incell_detail_thr.cb_test_max[i * test_data.screen_param.rx_num +
									j] = MaxValue_Vkey;
			}
		}
	}
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			test_data.incell_detail_thr.cb_test_min[i * test_data.screen_param.rx_num + j] =
			    MinValue;
		}

		if (i == ChannelNumTest_ChannelXNum) {
			for (j = 0; j < test_data.screen_param.rx_num; j++) {
				test_data.incell_detail_thr.cb_test_min[i * test_data.screen_param.rx_num +
									j] = MinValue_Vkey;
			}
		}
	}
...
void OnThreshold_VkAndVaRawDataSeparateTest(char *strIniFile)
{
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		for (j = 0; j < test_data.screen_param.rx_num; j++) {
			test_data.incell_detail_thr.rawdata_test_max[i * test_data.screen_param.rx_num + j] =
			    MaxValue;
		}
	...
	for (i = 0; i < test_data.screen_param.tx_num + 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.incell_detail_thr.rawdata_test_max[i * test_data.screen_param.rx_num + k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
