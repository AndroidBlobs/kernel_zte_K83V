...
int get_rawdata_incell(int *data)
{
	...
	if (ret) {
		FTS_TEST_SAVE_INFO("write AD to reg01 fail\n");
		return ret;
	}
	...
	if (ret) {
		FTS_TEST_SAVE_INFO("write AE to reg01 fail\n");
		return ret;
	}
 ...
 */
int weakshort_get_adc_data_incell(u8 ch_num, int byte_num, int *adc_buf)
{
	int ret = 0;
	u8 short_state = 0;
	int times = 0;
