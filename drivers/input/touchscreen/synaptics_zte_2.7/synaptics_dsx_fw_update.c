...
unsigned char *customer_ID = NULL;
#ifdef ONE_TP_MULTI_MODULE
extern char ts_config_id2;
#endif
...
int syna_get_config_in_bl_mode(struct rmi_config_id *config_id)
{
	...
	pr_info("%s: customer_ID = %s, customer_ID_temp = %s\n",
	...
	#ifdef ONE_TP_MULTI_MODULE
	snprintf(syna_bl_id, sizeof(syna_bl_id), &customer_ID[4]);
	retval = kstrtou8(syna_bl_id, 16, &ts_config_id2);
	if (retval)
		return retval;
	pr_notice("%s: ts_config_id2 is 0x%x.\n", __func__, ts_config_id2);
	#endif
...
int synaptics_get_f34_crtl_info(struct synaptics_rmi4_data *ts, unsigned int *chip_id,
	unsigned int *vendor_id, unsigned int *fw_version)
{
	unsigned int chipid_in_chip, vendorid_in_chip, fwver_in_chip;
	uint8_t buf[4];
	unsigned char chip_mode = 0, val = 0;

	synaptics_rmi4_reg_read(ts, ts->f34_ctrl_base_addr, buf, 4);

	/*read chip run mode, ui or bootloader mode?*/
	synaptics_rmi4_reg_read(ts, ts->f01_data_base_addr, &val, 1);
	if ((val & 0x40) == 0) {    /* ui mode. */
	...
	TPD_DMESG("%s: tpd f34 Type:0x%x, Partner:0x%x, Fw_rev:0x%x, in chip\n",
		__func__, chipid_in_chip, vendorid_in_chip, fwver_in_chip);
	if (chipid_in_chip == 0x00 || vendorid_in_chip == 0x00 ||
		chipid_in_chip == 0xff || vendorid_in_chip == 0xff ||
		fwver_in_chip == 0xffff || chip_mode == 1) {
		TPD_DMESG("%s: may have some problem.\n", __func__);
		fwver_in_chip = 0x00;
		vendorid_in_chip = 0xff;
	}
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	if ((socinfo_get_charger_flag() == 1) ||
		(socinfo_get_ftm_flag() == 1)) {
		dev_info(rmi4_data->pdev->dev.parent,
			"%s: offcharging or ftm mode, do not update fw!\n",
			__func__);
		syna_update_flag = 2;
		goto exit;
	} else
		dev_info(rmi4_data->pdev->dev.parent,
			"%s: start auto updating!\n",
			__func__);
...
#endif
int syna_tp_fw_upgrade(struct tpd_classdev_t *cdev, char *fw_name, int fwname_len)
{
	if ((fwname_len <= 1) || (fwname_len >= sizeof(fwname))) {
		pr_info("fw bin name's length(%d) fail", fwname_len);
		return -EINVAL;
	}
	memset(fwname, 0, sizeof(fwname));
	snprintf(fwname, sizeof(fwname), "%s", fw_name);
	fwname[fwname_len - 1] = '\0';
	pr_info("fwname is %s", fwname);
	syna_file_name = fwname;

	fwu->force_update = true;
	syna_update_flag = 1;
	if (fwu_start_reflash() == 0) {
		syna_update_flag = 2;
		pr_notice("%s: update success\n", __func__);
	} else {
		syna_update_flag = 3;
