...
#if defined(ZTE_TP_REPORT_BUTTON)
		switch (button) {
		case 0:
			x = 128;
			break;
	...
	int *p_fw_ver)
{
	...
	*p_fw_ver = ts->config_id.fw_ver;
	...
	if (strnstr(panel_name, VENDOR_ID1_NAME, 50))
		syna_file_name = SYN_VENDOR1_FW_NAME;
	else if (strnstr(panel_name, VENDOR_ID2_NAME, 50))
	...
	if (!bootloader_mode) {
		if (ts_config_id2 >= SYNA_VENDOR_ID1 && ts_config_id2 < SYNA_VENDOR_ID2)
			syna_file_name = SYN_VENDOR1_FW_NAME;
		else if (ts_config_id2 >= SYNA_VENDOR_ID2 && ts_config_id2 < 0x39)
			syna_file_name = SYN_VENDOR2_FW_NAME;
		else
			pr_err("%s: get syna_file_name by panel name error.\n", __func__);
	} else {
