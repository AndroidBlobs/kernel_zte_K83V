...
static DEVICE_ATTR(register_write,  0220, NULL, sx9325_register_write_store);
static DEVICE_ATTR(register_read, 0220, NULL, sx9325_register_read_store);
static DEVICE_ATTR(raw_data, 0444, sx9325_raw_data_show, NULL);
static DEVICE_ATTR(chipinfo, 0444, chipinfo_value_show, NULL);
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK)	{
			if (g_sx9310_chip) {
			    u8 touchStatus = 0;
				read_register(g_sx9310_chip, SX9310_IRQSTAT_REG, &touchStatus);
				pr_info("sx9310_fb_callback.FB_BLANK_UNBLANK for read status %d!\n", touchStatus);
...
MODULE_DEVICE_TABLE(of, sx9310_match_table);
