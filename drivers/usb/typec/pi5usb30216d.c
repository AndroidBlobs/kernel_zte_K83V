...
static int PI5USB30216D_read_blockdata(struct i2c_client *i2c, u8 reg, u8 length, u8 *dest)
{
	...
	#if 0
	printk("PI5USB30216D_read_blockdata ret(%d) length(%d)\n", ret, length);
	{
		int i = 0;
		for (i = 0; i < length; i++)
			printk("PI5USB30216D_read_blockdata data(%x)\n", dest[i]);
	}
	#endif
    return ret;
}
...
static int PI5USB30216D_write_blockdata(struct i2c_client *i2c, u8 reg, u8 length, u8 *dest)
{
	...
	/*printk("PI5USB30216D_write_blockdata ret(%d) length(%d)\n",ret,length);*/
    return ret;
}
...
static void process_register(struct PI5USB30216D_info *info, u8 *pi5usb302_reg_data)
{
	...
	/*printk("INT_ATTACHED_STATE(%x)\n",val);*/

    /* check cable dir */
    tmp = pi5usb302_reg_data[3];
    val = tmp & MOD_CC_CONNECT_STATE;
    info->type_c_param.cable_dir = val;
	/*printk("PI5USB30216D:MOD_CC_CONNECT_STATE(%x)\n",val);  0x01: cc1   0x10: cc2   */

    /* check attach state */
    tmp = pi5usb302_reg_data[3];
    val = (tmp & MOD_PORT_STATE) >> MOD_PORT_STATE_SHIFT ;
    info->type_c_param.attach_state = val;
	/*printk("PI5USB30216D:MOD_PORT_STATE(%x)\n",val);  0x01: device   0x10: host   */
	printk("PI5USB30216D attach(%x) mode(%x)\n", info->type_c_param.active_cable_attach, info->type_c_param.attach_state);
}
...
/*from mtk to qcom change,++*/
	...
	info->irq_gpio = retval;
			...
			if (!retval) {
			    ...
			    printk("PI5USB30216D eint_gpio(%d) irqnum(%d)\n", eint_gpio, info->irq_gpio);
			}
			#endif
/*from mtk to qcom change,-- */
		}
	} else {
		printk("PI5USB30216D cannot get the node\n");
		return 1;
	}
...
/*from mtk to qcom change,-- */
    retval = request_threaded_irq(info->irq_gpio, NULL, PI5USB30216D_irq_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT, "PI5USB30216D_irq", info);
	if (retval != 0) {
		printk("PI5USB30216D request_irq fail, ret %d, irqnum %d!!!\n", retval, info->irq_gpio);
		return 1;
	}
	return 0;
}
...
static int PI5USB30216D_check_id(struct PI5USB30216D_info *info)
{
	...
	if (pi5usb302_id != reg_val) {
		printk("PI5USB30216D_check_id fail(%x)\n", reg_val);
		return -ENODEV;
	}
	printk("PI5USB30216D_check_id sucess\n");
...
static int PI5USB30216D_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!PI5USB30216D_eint_init(info)) {
		printk("PI5USB30216D_eint_init success\n");
    }
	else {
	...
	usb_typec_id = 1;
	printk("PI5USB30216D usb type-c ship finish probe\n");
	return 0;
