...
#define CS_PDAF_E_ADDR 0xA94
#define CS_PDAF_ADDR   0xA95

#define CS_PDCC_S_ADDR 0xD9B
#define CS_PDCC_E_ADDR 0xD9C
#define CS_PDCC_ADDR   0xD9D
...
int ov13855_validflag_check_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->cal_data.mapdata[FLAG_PDCC_ADDR] != FLAG_VALID_VALUE) {
		pr_err("%s :%d: DCC flag invalid 0x%x\n",
			__func__, __LINE__, e_ctrl->cal_data.mapdata[FLAG_PDCC_ADDR]);
		flag |= 0x20;
	}
...
int ov13855_checksum_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	for (j = CS_PDCC_S_ADDR; j <= CS_PDCC_E_ADDR; j++)
		checksum += e_ctrl->cal_data.mapdata[j];

	if ((checksum % 256) != e_ctrl->cal_data.mapdata[CS_PDCC_ADDR]) {
...
static const struct of_device_id ov13855_eeprom_dt_match[] = {
	{ .compatible = "zte,ov13855_014-eeprom", .data = &ov13855_eeprom_func_tbl},
};
